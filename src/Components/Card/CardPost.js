...
const CardPost = ({ item = {}, type = 'posts', showButton = true, onClickBtn, editMode = false, onChange, isBtnLoading = false, isBtnDisabled = false }) => {
    return (
        <div key={item?.id} className="card flex-auto bg-base-100 shadow-xl w-full relative">            
            <div className={`card-body ${type === 'albums' && 'flex-row items-center'}`}>
                ...
                {!editMode ?
                    ...
                    :
                    <textarea onChange={e => onChange(e.target.value, 'body')} value={item?.bodu} className="textarea textarea-bordered rounded-lg resize-none h-32" placeholder="Body" />
                }                
                {showButton && !editMode &&
                ...
                {
                    editMode && <button disabled={isBtnLoading || isBtnDisabled} onClick={onClickBtn} className={`btn btn-primary rounded-lg normal-case self-end ${isBtnLoading && 'loading'}`}>Add</button>
                }
            </div>
        </div>
    );
